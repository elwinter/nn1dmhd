...
xt_train = np.loadtxt(os.path.join(problem, "xy_train.dat"))

# Extract the grid coordinates.
x_grid = xt_train[::hp.ny_train, 0]   # np.ndarray, shape (p.nx_train,)
t_grid = xt_train[:hp.ny_train, 1]  # np.ndarray, shape (p.nt_train,)
...
for (j, t) in enumerate(t_grid):
    ...
    for (i, variable_name) in enumerate(p.dependent_variable_names):
        ...
        x = x_grid
        y = ψ[i][j::hp.nx_train]
        y_a = ψ_a[i][j::hp.nx_train]
        y_err = ψ_err[i][j::hp.nx_train]
