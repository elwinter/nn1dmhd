...
# Visualize the results from a pde1bvp_coupled_pinn_3D.py run.
...
xyzt_train = np.loadtxt(os.path.join(runid, "xyzt_train.dat"))
x_train = xyzt_train[:, 0]
y_train = xyzt_train[:, 1]
z_train = xyzt_train[:, 2]
t_train = xyzt_train[:, 3]

# Extract the unique training point values.
x_train_vals = x_train[::hp.ny_train*hp.nz_train*hp.nt_train]
y_train_vals = y_train[:hp.ny_train*hp.nz_train*hp.nt_train:hp.ny_train*hp.nt_train]
z_train_vals = z_train[:hp.nz_train*hp.nt_train:hp.nt_train]
t_train_vals = t_train[:hp.nt_train]
n_x_train_vals = len(x_train_vals)
n_y_train_vals = len(y_train_vals)
n_z_train_vals = len(z_train_vals)
