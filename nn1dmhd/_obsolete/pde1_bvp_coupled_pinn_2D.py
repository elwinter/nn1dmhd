...
default_problem = (
    "/Users/winteel1/research_local/src/nn1dmhd/problems/static2.py"
)
...
def main():
    ...
    # Shapes are (n_train, 3), (n_train_in, 3), (n_train_bc, 3)
    xyt_train, xyt_train_in, xyt_train_bc = p.create_training_data_gridded(
        nx_train, ny_train, nt_train
    )
    # Shape is (n_train, p.n_dim)
    np.savetxt(os.path.join(output_dir, "xyt_train.dat"), xyt_train)
    n_train = len(xyt_train)
    # Shape is (n_train_in, p.n_dim)
    np.savetxt(os.path.join(output_dir, "xyt_train_in.dat"), xyt_train_in)
    n_train_in = len(xyt_train_in)
    # Shape is (n_train_bc, p.n_dim)
    np.savetxt(os.path.join(output_dir, "xyt_train_bc.dat"), xyt_train_bc)
    n_train_bc = len(xyt_train_bc)
    ...
    bc0 = p.compute_boundary_conditions(xyt_train_bc)
    ...
    for epoch in range(max_epochs):
        ...
        with tf.GradientTape(persistent=True) as tape0:
            with tf.GradientTape(persistent=True) as tape1:
                ...
                N_all = [model(xyt) for model in models]
                ...
                N_bc = [model(xyt_bc) for model in models]
            ...
            delN_all = [tape1.gradient(N, xyt) for N in N_all]
            ...
            # Each Tensor has shape (n_train, 1).
            G_all = [
                pde(xyt, N_all, delN_all) for pde in p.differential_equations
            ]
    ...
    # Shapes are ???
    with tf.GradientTape(persistent=True) as tape1:
        N_train = [model(xyt) for model in models]
