...
# Default number of training points in the z-dimension.
default_nz_train = 11

# Default number of validation points in the z-dimension.
default_nz_val = 101

# Default number of training points in the t-dimension.
default_nt_train = 11

# Default number of validation points in the t-dimension.
default_nt_val = 101

# Default the default problem file.
default_problem = "/Users/winteel1/research_local/src/nn1dmhd/nn1dmhd/problems/static3.py"
...
def create_command_line_argument_parser():
    ...
    parser.add_argument(
        "--nz_train", type=int, default=default_nz_train,
        help="Number of equally-spaced training points in z dimension (default: %(default)s)"
    )
    parser.add_argument(
        "--nz_val", type=int, default=default_nz_val,
        help="Number of equally-spaced validation points in z dimension (default: %(default)s)"
    )
    return parser
...
def main():
    ...
    # Shapes are (n_train, 4), (n_train_in, 4), (n_train_bc, 4)
    xyzt_train, xyzt_train_in, xyzt_train_bc = p.create_training_data_gridded(
        nx_train, ny_train, nz_train, nt_train
    )
    # Shape is (n_train, p.n_dim)
    np.savetxt(os.path.join(output_dir, "xyzt_train.dat"), xyzt_train)
    n_train = len(xyzt_train)
    # Shape is (n_train_in, p.n_dim)
    np.savetxt(os.path.join(output_dir, "xyzt_train_in.dat"), xyzt_train_in)
    n_train_in = len(xyzt_train_in)
    # Shape is (n_train_bc, p.n_dim)
    np.savetxt(os.path.join(output_dir, "xyzt_train_bc.dat"), xyzt_train_bc)
    n_train_bc = len(xyzt_train_bc)
    ...
    bc0 = p.compute_boundary_conditions(xyzt_train_bc)
    ...
    # Shape (n_train, 3)
    xyzt_train_var = tf.Variable(xyzt_train, dtype=precision)
    xyzt = xyzt_train_var
    # Shape (n_train_in, 3)
    xyzt_train_in_var = tf.Variable(xyzt_train_in, dtype=precision)
    xyzt_in = xyzt_train_in_var
    # Shape (n_train_bc, 3)
    xyzt_train_bc_var = tf.Variable(xyzt_train_bc, dtype=precision)
    xyzt_bc = xyzt_train_bc_var
    ...
    for epoch in range(max_epochs):
        ...
        with tf.GradientTape(persistent=True) as tape0:
            with tf.GradientTape(persistent=True) as tape1:
                ...
                N_all = [model(xyzt) for model in models]
                ...
                N_bc = [model(xyzt_bc) for model in models]
            ...
            delN_all = [tape1.gradient(N, xyzt) for N in N_all]
            ...
            # Each Tensor has shape (n_train, 1).
            G_all = [
                pde(xyzt, N_all, delN_all) for pde in p.differential_equations
            ]
    ...
    # Shapes are ???
    with tf.GradientTape(persistent=True) as tape1:
        N_train = [model(xyzt) for model in models]
    delN_train = [tape1.gradient(N, xyzt) for N in N_train]
    for i in range(p.n_var):
