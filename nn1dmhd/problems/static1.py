...
def create_training_data_gridded(nx:int, nt:int):
    ...
    Returns
    -------
    xt : np.ndarray, shape (nx*ny*nz*nt, n_dim)
...
def pde_ux(xt, ψ, del_ψ):
    ...
    G = (
        ρ*(dux_dt + ux*dux_dx) +
        dP_dx + (By*dBy_dx + Bz*dBz_dx)/μ0
    )
    return G


# @tf.function
def pde_uy(xt, ψ, del_ψ):
