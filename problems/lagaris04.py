"""Problem definition file for simple coupled ODE (Lagaris problem 4).

These ordinary differential equations are taken from the paper:
...
dimensions (independent variables) in the problem.
...
def y1_analytical(X):
    """Analytical solution lagaris04 y1.
...
def y2_analytical(X):
    """Analytical solution lagaris04 y2.
...
Y_analytical = [y1_analytical, y2_analytical]


def dy1_dx_analytical(X):
    """Analytical 1st derivative to lagaris04 y1.

    Analytical 1st derivative of y1 wrt x for lagaris04.
...
def dy2_dx_analytical(X):
    """Analytical 1st derivative to lagaris04 y2.

    Analytical 1st derivative of y2 wrt x for lagaris04.
...
dY_dX_analytical = [dy1_dx_analytical, dy2_dx_analytical]


def de1(X, Y, dY_dX):
    """ODE 1 (Example 4 from Lagaris et al (1998)).

    Compute the 1st-order ODE 1 from Example 4 from Lagaris et al (1998)) in
    standard form.
    ...
    """
    n = X.shape[0]
    # x, y1, y2, dy1_dx, dy2_dx are shape (n,).
    x = X[:, 0]
    y1 = Y[0][:, 0]
    y2 = Y[1][:, 0]
    dy1_dx = dY_dX[0][:, 0]
...
def de2(X, Y, dY_dX):
    """ODE 2 (Example 4 from Lagaris et al (1998)).

    Compute the 1st-order ODE 2 from Example 4 from Lagaris et al (1998)) in
    standard form.
